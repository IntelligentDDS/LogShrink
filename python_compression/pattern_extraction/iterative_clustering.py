...
sys.path.append("../")
from pattern_extraction.sampling import random_sampling
from pattern_extraction.similarity_function import *
...
def iterative_clustering_seq(input_data=None,
    ...
    while zero_count <= terminated_num and len(mismatch_raw_index) > 1:
        ...
        sampled_ids, sampled_data = random_sampling(mismatch_raw_index, sample_rate)
        # ordered
        clusters = clustering(input_data[sampled_data], onehot_encoding_all[sampled_data], threshold, templates_number)
        repres = repres_extracting(clusters, cluster_min_len)

        if len(repres) == 0:
            zero_count += 1
            continue
        mismatch_raw_index, cluster_result = matching(repres, onehot_encoding_all[mismatch_raw_index], threshold, mismatch_raw_index)
        ...
        cluster_result = {k + len(centers): val for k, val in cluster_result.items()}
...
def matching(centers, input_data, threshold, raw_index):
    ...
    validMinList = distance_matrix[np.arange(len(min_index_list)), min_index_list] < threshold
    clu_result = np.array([min_index_list[i] if x else -1 for i, x in enumerate(validMinList)])

    # get the mismatched data with its index
    if -1 in clu_result:
        mismatch_index = np.where(clu_result == -1)[0]  # mismatched sequence indexes
    else:
        mismatch_index = []
    ...
    mismatch_raw_index = raw_index[mismatch_index]
    t2 = time.time()
    log("debug", '------matching takes %.15f seconds, %d sequences are not matched' % ( t2- t1, len(mismatch_index)))
